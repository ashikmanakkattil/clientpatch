...
:copyLogs
:: Define the source path for Logs (local folder)
set logsSourcePath=C:\hydedu\Logs

:: Define the destination path on C: drive for Logs
set logsDestPath=C:\Logs

:: Check if Logs destination folder exists, if not create it
if not exist "%logsDestPath%" (
    mkdir "%logsDestPath%"
)

:: Copy all files and subfolders from the source folder (Logs) to the destination
xcopy "%logsSourcePath%" "%logsDestPath%" /E /I /Y
echo Logs folder copied successfully to %logsDestPath%.

:copyNMS
:: Define the source path for NMS (local folder)
set nmsSourcePath=C:\hydedu\NMS

:: Define the destination path on C: drive for NMS
set nmsDestPath=C:\NMS

:: Check if NMS destination folder exists, if not create it
if not exist "%nmsDestPath%" (
    mkdir "%nmsDestPath%"
)

:: Copy all files and subfolders from the source folder (NMS) to the destination
xcopy "%nmsSourcePath%" "%nmsDestPath%" /E /I /Y
echo NMS folder copied successfully to %nmsDestPath%.

:: Pause to view the result


xcopy "C:\hydedu\TinyWall-v3-Installer.msi" "C:\" /f /y
...
xcopy "C:\hydedu\uninstall.bat" "%USERPROFILE%\Desktop\" /f /y


xcopy "C:\hydedu\uninstall.bat" "C:\" /f /y


xcopy "C:\hydedu\Logs.bat" "%USERPROFILE%\Desktop\" /f /y
...
:: ==========================================
msiexec /i "C:\hydedu\Sepoy_CLIENT_SETUP_2024_DEC_V1.msi" /qn /norestart >nul 2>&1
...
:: START OF COPY CLIENT.EXE.config FILE FOR AUTO-ENTERING THE SERVER IP (Change as required)
:: =========================================================================================
...
IF %OS%==WIN7 IF %BIT%==32BIT (
    ...
    xcopy "C:\hydedu\ClientExePro.exe.config" "C:\Program Files\Sepoy_CLIENT_SETUP_2024" /f /y
)
...
IF %OS%==WIN7 IF %BIT%==64BIT (
    ...
    xcopy "C:\hydedu\ClientExePro.exe.config" "C:\Program Files (x86)\Sepoy_CLIENT_SETUP_2024" /f /y
)
